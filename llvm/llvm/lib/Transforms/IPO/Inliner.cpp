...
STATISTIC(NumCallsite, "Number of callsite");

static cl::opt<bool> InlineNewCallsite("inline-new-callsite",
          cl::init(true), cl::Hidden,
          cl::desc("when a function is inlined, consider new callsite"));

static cl::opt<std::string> InlineStrategy(
    "inline-strategy", cl::init(""), cl::value_desc("inlineStrategy"),
    cl::desc(
        "inline strategy "),
    cl::Hidden);
...
PreservedAnalyses InlinerPass::run(LazyCallGraph::SCC &InitialC,
  ...
  // LLVM_DEBUG(dbgs() <<debug_count++ << "hello debug\n " );
  ...
  for (int I = 0; I < (int)Calls.size(); ++I) {
    ...
    for (; I < (int)Calls.size() && Calls[I].first->getCaller() == &F; ++I) {
      ...
      // errs() << "if-inline:" <<"\n";
      if(!InlineStrategy.empty()){
        if(InlineStrategy[0]=='0'){
          Advice->recordUnattemptedInlining();
          InlineStrategy=InlineStrategy.substr(1);
          continue;
        }
        InlineStrategy=InlineStrategy.substr(1);
      }else{
