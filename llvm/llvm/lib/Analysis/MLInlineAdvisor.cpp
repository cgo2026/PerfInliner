...
std::unique_ptr<InlineAdvice> MLInlineAdvisor::getAdviceImpl(CallBase &CB) {
  ...
  *ModelRunner->getTensor<int64_t>(FeatureIndex::cost_estimate) = CostEstimate;
  ...
  auto &BFI = FAM.getResult<BlockFrequencyAnalysis>(Callee);
  int64_t BlockFrequency = BFI.getBlockFreq(&Callee.getEntryBlock()).getFrequency();
  *ModelRunner->getTensor<int64_t>(FeatureIndex::block_frequency) =
      BlockFrequency;
      
  *ModelRunner->getTensor<int64_t>(FeatureIndex::top_loop_level) =
      CalleeBefore.TopLevelLoopCount;

  int64_t max_caller_caller_basic_block_count = 0;
  int64_t max_caller_caller_conditionally_executed_blocks = 0;
  int64_t if_caller_in_loop = 0;

  CallGraph CGraph(M);
  if (CallGraphNode *CallerNode = CGraph[&Caller]) {
      for (auto &CallRecord : *CallerNode) {
        if (Function *CallerOfCaller = CallRecord.second->getFunction()) {
          if (CallerOfCaller->isDeclaration())
            continue;
          auto &CallerOfCaller1 = getCachedFPI(*CallerOfCaller);
          max_caller_caller_basic_block_count = std::max(max_caller_caller_basic_block_count, CallerOfCaller1.BasicBlockCount);
          max_caller_caller_conditionally_executed_blocks = std::max(max_caller_caller_conditionally_executed_blocks, CallerOfCaller1.BlocksReachedFromConditionalInstruction);

          LoopInfo &LI = FAM.getResult<LoopAnalysis>(*CallerOfCaller);
          for(Loop *L : LI){
            for(BasicBlock *BB : L->blocks()){
              for(Instruction &I : *BB){
                if(auto *callbase = dyn_cast<CallInst>(&I)){
                  if(callbase->getCalledFunction() == &Caller){
                    if_caller_in_loop = 1;
                    break;
                  }
                }
  ...
  *ModelRunner->getTensor<int64_t>(FeatureIndex::max_caller_caller_basic_block_count) = max_caller_caller_basic_block_count;
  *ModelRunner->getTensor<int64_t>(FeatureIndex::max_caller_caller_conditionally_executed_blocks) = max_caller_caller_conditionally_executed_blocks;
  *ModelRunner->getTensor<int64_t>(FeatureIndex::if_caller_in_loop) = if_caller_in_loop;
