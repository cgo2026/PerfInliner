...
static cl::opt<std::string> HarewareEnventsFile(
    "hareware-events-file", cl::init(""), 
    cl::desc(
        "the path of hareware envents "),
    cl::Hidden);
...
std::unordered_map<std::string, std::vector<int>> readHardwareEventFile(const std::string& filename){
  std::unordered_map<std::string, std::vector<int>> hardwareEvents;
  std::ifstream file(filename);
  if (!file.is_open()) {
      errs() << "cannot read: " << filename << "\n";
      return {};
  }
  std::string line;
  ...
  while (std::getline(file, line)) {
    ...
    if (isHeader) {
      ...
      if (header != "Symbol") {
        errs() << "The file format is incorrect, and the Symbol header is missing" << "\n";
        return {};
      }

      std::string event;
      while (std::getline(iss, event, ',')) {
...
bool InlineAdvisorAnalysis::Result::tryCreate(
  ...
  if(HarewareEnventsFile != ""){
    
    Advisor->setHardwareEvents(readHardwareEventFile(HarewareEnventsFile));
  }
  return !!Advisor;
}
