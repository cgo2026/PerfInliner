...
public:
  ...
  void setHardwareEvents(const std::unordered_map<std::string, std::vector<int>> &HWEvents) {
      hardwareEvents = HWEvents;
  }
...
public:
  ...
  struct Result {
    ...
    // std::unordered_map<std::string, std::vector<double>> getHarewareEvents() {return hardwareEvents;}
